...
SELECT 
   ...
   -- HACK to get a path for threat case generation)
   (SELECT object FROM sophos_process_activity spa WHERE spa.SophosPID = Full_Tree.SophosPID AND spa.subject IN ('FileBinaryChanges', 'FileDataChanges', 'FileOtherChanges') AND spa.time >= Full_Tree.starttime AND spa.time < Full_Tree.starttime + 3600 LIMIT 1) 'Generate Threat Case?',
