SELECT
 ...
 WHEN (SELECT
  ...
  AND name LIKE 'service.%' AND data IN (1,2)) IS NULL THEN 'NONE ✅'
  ELSE
  (SELECT
    CAST(group_concat(name, CHAR(10)) AS TEXT) AS "bad services"
  FROM
  ...
  SELECT
    ...
    (SELECT
       CASE
          WHEN data = 1 THEN 'GOOD ✅'
          WHEN data = 2 THEN 'SUSPICIOUS ⚠️' || (SELECT (json_extract(raw,'$.threatName') || ' --detected-- ' || json_extract(raw,'$.location')) AS 'Threat Details' FROM sophos_events_summary WHERE id IN (SELECT
