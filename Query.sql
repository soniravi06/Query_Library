...
WITH Activity_List AS (
  SELECT
   ...
   CASE
    WHEN spa.sophosPID LIKE '4:%' THEN 'SYSTEM'
   ELSE (
     SELECT processname FROM sophos_process_Journal spj
      WHERE spj.sophosPID = spa.SophosPID)
   END Process_Name,
   CAST(GROUP_CONCAT(DISTINCT action ) AS TEXT) Action,
   object,
   CASE
    WHEN (
      SELECT path FROM file f
      WHERE f.path = spa.object) > '' THEN 1
      ELSE 0
    END On_Disk,
